...
class YoloFusion(lnn.module.Darknet):
    """ Yolo v2 that uses midway fusion to process multi-channel input images.
    ...
    def __init__(self, num_classes=20, weights_file=None, conf_thresh=.25, nms_thresh=.4, input_channels=3, fusion_channels=1,
        ...
        if self.input_channels < 1 or self.fusion_channels < 1:
    ...
    def _forward(self, x):
        ...
        # Fusion
        outputs.append(self.layers[0](fusion))

        # Main
        outputs.append(self.layers[1](main))
        outputs.append(self.layers[2](outputs[1]))
        outputs.append(self.layers[3](outputs[1]))

        # Combination
        out = self.layers[4](torch.cat((outputs[3], outputs[2], outputs[0]), 1))
