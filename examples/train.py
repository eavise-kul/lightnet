...
LR_SCALES = (10, 0.1, 0.1)          # Scales to scale the inital learning rate with
...
def test(net, dataset, final=False):
    """ Test the network """
    original_dim = net.input_dim
    net.input_dim = NETWORK_SIZE
    net.eval()
    kwargs = {'num_workers': 4, 'pin_memory': False} if args.cuda else {}
    loader = torch.utils.data.DataLoader(dataset, batch_size=BATCH//BATCH_SUBDIV, collate_fn=ln.bbb_collate, **kwargs)

    at = lnt.AnnoToTensor(net)
    ...
    for _, (data, target) in enumerate(loader):
        ...
        for i in range(len(target)):
            key = len(anno)
            anno[key] = target[i]
            det[key] = ln.bbox_to_brambox(output[i], net.input_dim, class_label_map=names)
            num_det += len(det[key])
    ...
    if num_det > 1:
        ...
        while (len(det[i]) <= 0):
            i += 1
        img = np.array((tf.ToPILImage()(dataset.img_tf(cv2.imread(dataset.id(dataset.keys[i]))))))
        bbb.draw_box(img, anno[i], color=(0,0,255), inline=True)
        detection = [det for det in det[i] if det.confidence >= .5]
        bbb.draw_box(img, det[i], color=(0,255,0), inline=True)
        if args.visdom:
            viz.image(img, win='example-img')
        else:
            cv2.imwrite('.sandbox/image.png', img)
        ##TEMP
...
if __name__ == '__main__':
    ...
    trainset = ln.DarknetData(args.train, network)
    if args.test is not None:
        testset = ln.DarknetData(args.test, network, train=False, augment=False, class_label_map=names)
    print()
